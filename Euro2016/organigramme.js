var dejaGenere = false; //Afin de ne pas générer plusieurs fois les pays qualifiés
var compteTour = 8; //Par défaut, c'est les 8ème de finale
var paysRestants = []; //Liste des pays encore en lice

function selectionnerPays(){
	/*Génère aléatoirement la liste des pays sélectionnés pour le tableau d'éliminatoires
	Note : Les pays sont sélectionnés aléatoirement sans respecter les groupes.
	*/
...
document.getElementById("generer-equipes").onclick=function(){//On ne s'en sert qu'ici, donc on utilise une fonction anonyme, elle n'a pas besoin d'avoir de nom.
	/*Génère et affiche la liste des pays qualifiés au click
	*/
	if(!dejaGenere){
...
function passerTourSuivant(){
	/*Génère la liste des pays qualifiés pour le tour suivant
	*/
	var gagnants =[] ;
...
document.getElementById("tour-suivant").onclick=function() {
	/* Au click, affiche la liste des pays vainqueurs du tour précédent
	*/
	if(paysRestants.length>1){ //Si on n'a pas déjà tout généré
		...
		if(compteTour===8){ //En fonction de l'avancement de la compétition, on récupère les id des cases à remplir
			...
			//On récupère le nom du pays gagnant et on joue le fichier mp3 correpondant
